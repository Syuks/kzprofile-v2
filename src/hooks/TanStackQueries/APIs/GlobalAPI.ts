...
// New maps records are not being returned on player profiles.
// The creator of gokz.top has provided kzprofile and kzgo.eu access to gokz.top API to fix this.
const gokzTopAPI = {
  baseURL: "https://api.gokz.top/v0",
}
 ...
 *
 */
export const GlobalAPI_GetRecordsTop = (params: GetRecordsTopParams, gokzTop: boolean = false) => {
  const url = "records/top"

  const queryString = generateUrlSearchParams(params).toString()

  return fetch(`${gokzTop ? gokzTopAPI.baseURL : globalAPI.baseURL}/${url}?${queryString}`)
}
