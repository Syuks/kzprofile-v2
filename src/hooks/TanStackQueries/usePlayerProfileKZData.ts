...
const playerProfileKZDataQueryOptions = (steamID: string, gameMode: GameMode, gokzTop: boolean) => {
  return queryOptions({
    ...
    queryFn: async () => {
      ...
      const [globalMaps, playerTimes] = await Promise.all([
        fetchKZProfileMaps(),
        fetchPlayerTimes(steamID, gameMode, gokzTop),
      ])
...
const usePlayerProfileKZData = (steamID: string, gameMode: GameMode, gokzTop: boolean) => {
  return useQuery(playerProfileKZDataQueryOptions(steamID, gameMode, gokzTop))
}

const fetchPlayerProfileKZData = (steamID: string, gameMode: GameMode, gokzTop: boolean) => {
  return queryClient.fetchQuery(playerProfileKZDataQueryOptions(steamID, gameMode, gokzTop))
}
