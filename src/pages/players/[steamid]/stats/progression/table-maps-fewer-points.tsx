...
interface Progression_TableMapFewerPointsProps {
    recordsTopStatistics: RecordsTopStatistics
    className?: string
}

function Progression_TableMapFewerPoints({
    recordsTopStatistics,
    className,
}: Progression_TableMapFewerPointsProps) {
    const [localSettings] = useLocalSettings()

    const [runType] = useRunType()

    const mapsWithFewerPoints = useMemo<RecordTopStat[]>(() => {
        return [...recordsTopStatistics.finishes[runType]]
            .sort((finishA, finishB) => finishA.points - finishB.points)
            .slice(0, 6)
    ...
    return (
        <Card className={className}>
            <CardHeader>
                <CardTitle>Maps with fewer points</CardTitle>
            </CardHeader>
            <CardContent>
                <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                        {mapsWithFewerPoints.map((finish, index) => {
...
export default Progression_TableMapFewerPoints
